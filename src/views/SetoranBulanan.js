...
import InputCurrency from './Components/CurrencyInputSetoranBulanan'

class SetoranBulanan extends Component {
  constructor(props) {
    super(props);
    this.state = {
      jenisTempat: '',
  ...
  componentDidMount() {
    let biaya_harian = this.props.biayaRetribusi.biaya_retribusi.filter(
      (item) => {
        return item.jenis_pembayaran == 'Bulanan';
      },
    );
        ...
        <View>
          ...
          <View style={styles.input}>
            <Picker
              ...
              <Picker.Item label="Pilih Jenis Tempat" value={-1} key={-1} />
          ...
          <InputCurrency calculateSallary={this.handleJumlah} />
...
export default connect(mapStateToProps)(SetoranBulanan);
